<form method="post" action="{{ $action }}" enctype="multipart/form-data">
    ...
    <div class="col-lg-6 col-md-11 col-sm-12 mb-3">
        ...
        <div class="form-group col-12 col-md-6">
            ...
            <script>
                // checkbox bindings
                document.getElementById('enable_devoir').addEventListener('change', e => toggleWeight('poids_devoir',
                    e.target.checked));
                document.getElementById('enable_interrogation').addEventListener('change', e =>
                    toggleWeight('poids_interrogation', e.target.checked));
                document.getElementById('enable_examen').addEventListener('change', e => toggleWeight('poids_examen',
                    e.target.checked));
                document.getElementById('enable_tp').addEventListener('change', e => toggleWeight('poids_tp',
                    e.target.checked));
                document.getElementById('enable_expose').addEventListener('change', e => toggleWeight('poids_expose',
                    e.target.checked));
            </script>
        ...
        <div class="form-group col-12 col-md-6">
            ...
            <input type="number" min="1" class="form-control" id="coefficient" step="1" name="coefficient"
                aria-describedby="coefficient" placeholder="Coefficient de la matière"
                value="{{ old('coefficient', $uv->coefficient) }}">
            {!! errorAlert($errors->first('coefficient'), 'coefficient') !!}
        </div>

        <div class="form-group col-12 col-md-6">
            <x-forms.label content="CM de la matière" for="cm" required="{{ true }}" />
            <input type="number" min="1" class="form-control" id="cm" step="1" name="cm"
                aria-describedby="cm" placeholder="cm de la matière" value="{{ old('cm', $uv->cm) }}">
            {!! errorAlert($errors->first('cm'), 'cm') !!}
        </div>

        <div class="form-group col-12 col-md-6">
            <x-forms.label content="Volume horaire des Travaux Dirigés de l'UE" for="td"
                required="{{ true }}" />
            <input type="number" min="1" class="form-control" id="td" step="1" name="td"
                aria-describedby="td" placeholder="td de la matière" value="{{ old('td', $uv->td) }}">
            {!! errorAlert($errors->first('td'), 'td') !!}
        </div>

        <div class="form-group col-12 col-md-6">
            <x-forms.label content="Volume horaire des Travaux Pratiques de l'UE" for="tp"
                required="{{ true }}" />
            <input type="number" min="1" class="form-control" id="tp" step="1" name="tp"
                aria-describedby="tp" placeholder="tp de la matière" value="{{ old('tp', $uv->tp) }}">
            {!! errorAlert($errors->first('tp'), 'tp') !!}
        </div>

        <div class="form-group col-12 col-md-6">
            <x-forms.label content="EC de l'UE" for="ec" required="{{ true }}" />
            <input type="number" min="1" class="form-control" id="ec" name="ec"
                aria-describedby="ec" placeholder="ec de la matière" value="{{ old('ec', $uv->ec) }}">
            {!! errorAlert($errors->first('ec'), 'ec') !!}
        </div>

        <div class="col-lg-6 col-md-11 col-sm-12">
            <x-forms.label content="Enseignant(s) de la matière" for="enseignant_id"
                required="{{ true }}" />
    ...
    <div class="row mb-4">
        ...
        <div class="form-group col-6 col-md-2">
            ...
            <input type="number" class="form-control" min="1" max="100" name="poids_devoir"
                id="poids_devoir" value="{{ old('poids_devoir') }}">
        </div>
        {{-- <div class="form-group col-6 col-md-2">
