@extends('comptable.base', [
    'title' => 'Paiement',
    'page_name' => 'Historique de paiement',
    'breadcrumbs' => ['Etudiants', 'Historique de paiement'],
])
@section('bases')
    <div id="calendar"></div>
                ...
                eventClick: function(info) {
                            ...
                            fetch(`/espace-enseignant/absences/${window.currentEmploiDuTempsId}`)
                                ...
                                .then(absences => {
                                    document.querySelectorAll('#tablePresence tbody tr').forEach(row => {
                                        ...
                                        const absence = absences.find(a => a.etudiant_id == etuId);
                                        if (absence) {
                                            row.querySelector('.absent-check').checked = true;
                                            row.querySelector('input[type="text"]').value = absence.motif ?? '';
                                        } else {
                                            row.querySelector('.absent-check').checked = false;
                    ...
                    fetch(`/espace-enseignant/absences/${window.currentEmploiDuTempsId}`)
                        ...
                        .then(data => {
                            ...
                            document.querySelectorAll('#tablePresence tbody tr').forEach(row => {
                                const etuId = row.dataset.etudiantId;
                                const absence = data.find(a => a.etudiant_id == etuId);
                                if (absence) {
                                    row.querySelector('.absent-check').checked = true;
                                    row.querySelector('input[type="text"]').value = absence.motif ?? '';
                                } else {
