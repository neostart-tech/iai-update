...
use App\Http\Resources\TranchePaiementResource;
...
class TranchePaiementController extends Controller
{
    ...
    {
        return TranchePaiementResource::collection(TranchePaiement::latest()->get());
        // return view('comptabilite.Tranche._index');
        // return TranchePaiementResource::collection();
    }

     public function show(FraisScolarite $frais)
    {
        return new FraisScolariteResource($frais->tranchepaiement);
        // return view('comptabilite.Tranche._index');
        // return TranchePaiementResource::collection();
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($total > $frais->montant) {
            ...
            return abort('404', 'La somme des tranches dépasse le montant du frais. Il vous reste seulement' . $reste . ' F disponible');
        ...
        return __200('Tranches enregistrées avec succès.');
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($nouveauTotal > $tranche->fraiscolarite->montant) {
            return response()->json([
                "error" => "La somme des tranches dépasse le montant total autorisé."
            ]);
        }

                                        // return response()->json($tranche->fraiscolarite);
